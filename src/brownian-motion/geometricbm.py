...
class Analytical:
	...
	def theoretical_ev(self):
		return self.s_initial * np.exp(self.mu * (self.t_max - self.dt))
		
			
	def random_term(self):
		return ((self.mu - ((self.sigma)**2)/2) * self.dt) + (self.sigma * (np.sqrt(self.dt)) * random.gauss(0,1))


	def get_all_random_terms(self):
		random_terms = []
		for i,_ in enumerate(self.timesteps):
			if _ == self.timesteps[-1]:
				break
			random_terms.append(self.random_term())
		return random_terms


	def gbm_sim(self,sim_num):
		random_terms = self.get_all_random_terms()
